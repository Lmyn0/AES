...
void expand(unsigned char *PRK, size_t plen, unsigned char *info, size_t ilen, uint32_t L, unsigned char *OKM){
    ...
    memcpy(OKM + 32, T, 32);
    // T3 생성
    unsigned char message2[32 + ilen + 1];
    size_t m2len = 32 + ilen + 1;
    memcpy(message2, T, 32);
    memcpy(message2 + 32, info, ilen);
    message2[32 + ilen] = 0x03;

    HMAC(PRK, plen, message2, m2len, T);
    memcpy(OKM + 64, T, L-64);
